...
class SpecView
{
     ...
     * @access    public
     * @param     int  $isremote  是否远程
     * @param		dir	html文件生成目录
     * @return    string
     */
    function MakeHtmlByChannel($isremote=0,$channelid=1,$templet='spec_list.htm',$dir)
    {
        global $cfg_remote_site;
        //初步给固定值的标记赋值
        $this->ParseTempletsFirst();
        ...
        for($this->PageNo=1;$this->PageNo<=$totalpage;$this->PageNo++)
        {
            foreach($this->dtp->CTags as $tagid=>$ctag)
            {
                ...
                if($ctag->GetName()!="list")
                {
                    ...
                    if(trim($ctag->GetInnerText())=="")
                    {
                        $InnerText = GetSysTemplets($templet);
                    }
                    else
                    {
                        $InnerText = trim($ctag->GetInnerText());
                    ...
                    $this->dtp->Assign($tagid,
                    $this->GetArcListByChannel($limitstart,$row,
                    $ctag->GetAtt("col"),
                    $ctag->GetAtt("titlelen"),
                    $ctag->GetAtt("infolen"),
                    $ctag->GetAtt("imgwidth"),
            ...
            $makeFile = $dir."/spec_".$this->PageNo.$GLOBALS['art_shortname'];
            $murl = $makeFile;
            $makeFile = $GLOBALS['cfg_basedir'].$makeFile;
            $this->dtp->SaveTo($makeFile);
            //如果启用远程站点则上传
        ...
        copy($GLOBALS['cfg_basedir'].$dir."/spec_1".$GLOBALS['art_shortname'],$GLOBALS['cfg_basedir'].$dir."/index.html");
        $murl = $dir."/index.html";
        return $murl;
    }
    ...
    function GetArcListByChannel($limitstart=0,$row=10,$col=1,$titlelen=30,$infolen=250,
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$templet='spec_list.htm')
    {
        $typeid=$this->TypeID;
        if($row=="")
        {
