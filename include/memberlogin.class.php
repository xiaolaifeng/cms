...
// 使用缓存助手
helper('cache');
function CheckMobile($mobile,$msgtitle='手机号',$ckhas=true)
{
	return 'ok';
}
function CheckQQOpenid($openid,$msgtitle='qq号',$ckhas=true)
{
	//检测用户是否绑定过qq
	$row = $dsql->GetOne("SELECT * FROM `#@__member` WHERE openid LIKE '$openid' ");
...
class MemberLogin
{
    ...
    function CheckAuthUser(&$openid, $token)
    {
        global $dsql;
        ...
        //matt=10 是管理员关连的前台帐号，为了安全起见，这个帐号只能从后台登录，不能直接从前台登录
        $row = $dsql->GetOne("SELECT m.mid,m.matt,m.accesstoken,m.logintime FROM `#@__member` as m left join `#@__member_model_auth` as a on m.mid=a.mid WHERE  a.openid LIKE '$openid' ");
        if(is_array($row))
        {
            $this->PutLoginInfo($row['mid'], $row['logintime']);
            return 1;
