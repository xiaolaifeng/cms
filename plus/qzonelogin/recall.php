...
if($action == ''){
	...
	if($action == 'reg'){
		...
		if(empty($userid)){
			...
			exit();
		}elseif(strlen($userid) < $cfg_mb_idmin || strlen($userid) > 50){
			ShowMsg("用户名长度应该在{$cfg_mb_idmin}至50之间",-1);
			exit();
		}else{
		...
		if($dsql->ExecuteNoneQuery($inQuery)){//如果注册成功
			...
			$dsql->ExecuteNoneQuery("INSERT INTO `#@__member_flink`(mid,title,url) VALUES('$mid','文章投阅网','http://www.touyue.net'); ");					
            ...
            if($uid > 0) {
                ...
                if(!$rs) {
                    ...
                    if($res) {
                        ...
                        3 => "INSERT INTO `#@__member_flink` SET `mid`='$mid', `title`='文章投阅网', `url`='http://www.touyue.net';"
                        );                        
