...
if($dopost!='save')
{
    ...
    //处理软件版本表
    $query = "SELECT * FROM `#@__channeltype` WHERE id=-17";
    $cVersions = $dsql->GetOne($query);
    if(!is_array($cVersions))
    {
    	ShowMsg("读取历史版本出错!","javascript:;");
    	exit();
    }
    $addtable = $cVersions['addtable'];
    $addQuery = "SELECT * FROM `$addtable` WHERE parent_id='$aid'";
    $addRowVersion = $dsql->GetOne($addQuery);
    if(!empty($addRowVersion))
    {
    	unset($addRowVersion["parent_id"]);
    }
    $addRowVersion=XSSClean($addRowVersion);
...
else if($dopost=='save')
{
    ...
    $app_version= cn_substrR($app_version,40);
