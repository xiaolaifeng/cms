...
if($step == 1)
{
    ...
    if($dopost=='regbase')
    {
    	...
    	if(is_array($row))
    	{
    		ShowMsg("你已绑定过微博号{$openid} 已存在，请使用别的！", "-1");
    		exit();
    	}
        ...
        if(is_array($row))
        {
            ShowMsg("你指定的用户{$userid} 已存在，请使用别的！", "-1");
            exit();
        }
        ...
        `matt`, `spacesta` ,`face` ,`jointime` ,`joinip` ,`logintime` ,`loginip`,`sex` )
       VALUES ('$openid','$accesstoken','$mtype','$userid','$uname','10','$dfmoney','$dfscores',
       '0','$spaceSta','$face','$jointime','$joinip','$logintime','$loginip','$sex'); ";
         echo '00';
