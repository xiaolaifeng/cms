...
class FreeList
{
    ...
    function GetPageListST($list_len, $listitem="info,index,end,pre,next,pageno")
    {
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a class='last-one' href='".str_replace("{page}", $prepagenum, $tnamerule)."'>上一页</a>\r\n";
            $indexpage="<a class ='first-page'  href='".str_replace("{page}", 1, $tnamerule)."'>回到首页</a>\r\n";
        }
        else
        {
            $indexpage="<a class ='first-page' href='#'>回到首页</a>\r\n";
        }

        //下一页,未页的链接
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a  class='next-one'  href='".str_replace("{page}",$nextpagenum,$tnamerule)."'>下一页</a>\r\n";
...
//             $plist .= $optionlist;
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($this->PageNo != 1)
        {
            ...
            $indexpage="<a href='".$purl."PageNo=1'>回到首页</a>\r\n";
        }
        else
        {
            $indexpage="<a class='first-page' href='#'>回到首页</a>\r\n";
        ...
        //todu 去掉末页 共几页
        $plist .= $indexpage.$prepage.$listdd.$nextpage;
